//approch is right and need to read in next i just remove sorting to takedown sorting time complexity using vector
// class Solution {
// public:
...
public:
    int leastInterval(vector<char>& tasks, int n) {
        ...
        while(!pq.empty() || !q.empty()){
            ...
            // add ALL tasks whose cooldown is over
            while(!q.empty() && q.front().second <= anstime){
                pq.push(q.front().first);
                q.pop();
            }
